...
function ViewReport() {
  ...
  const [surat , setNewSurats] = useState('')
  const getStudentdetails = () => {
    axios
      ...
      .then(function (response) {
        setNewSurats(response.data.data.suratPercentage);
...
console.log("this is a surat" , surat)
  return (
              ...
              <div className="row mb-5 gap-2">
                <div className="start pt-2 col-md-4">
                  <DatePicker
                    ...
                    onChange={(date) => {
                      if (reportState.endDaterep && date > reportState.endDaterep) {
                        notification.error({
                          message: "Error",
                ...
                <div className="end pt-2 col-md-4">
                  <DatePicker
                    ...
                    onChange={(date) =>{
                      if (reportState.startDaterep && date < reportState.startDaterep) {
                        notification.error({message:"error" ,description: "End date cannot be earlier than start date"});
                      } 
                      else {
                      setreportState({
                        ...reportState,
                  ...
                  data.data.map((dat, datIndex) => (
                    ...
                    >
                      <div
                        ...
                        style={{ fontSize: "18px",padding:"10px" ,  fontWeight: "400"  , backgroundColor:"#E8EFFB"}} >
                        {dat.categoryName}
                      </div>
                      <div className="card-bodyy test overflow-scroll-width">
                          ...
                          <table className="table table-bordered">
                                  ...
                                  aadatIndex <= 0
                                    ? aadat.submissions.map(
                                        (submission, submissionHeaderIndex) => (
                                          ...
                                          >
                                          {submission?.fieldName?.charAt(0)?.toUpperCase() + submission?.fieldName?.slice(1)}
                            ...
                            <tbody>
                              {dat.aadats.map((aadat, aadatIndex) => (
                                <>
                                  <tr>
                                    <td className="text-center">{aadat.aadatName} </td>
                                    {aadat.submissions.map(
