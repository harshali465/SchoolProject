...
import "moment/locale/en-gb"; // Ensure English locale
import "moment-hijri";
...
function StudentView() {
  ...
  const hijriMonths = [
    "Muharram-ul-Haram",
    "SafaSafar-ul-Muzaffarr",
    "Rabi-ul-Awwal",
    "Rabi-ul-Akhar",
    "Jamadil Awwal",
    "Jamadil Ukhra",
    "Rajab-ul-Asab",
    "Shabaan-ul-Karim", // As requested
    "Ramazan-ul-Moazzam",
    "Shawwal-ul-Mukarram",
    "Zilqadal Haram",
    "Zilhajjil Haram",
  ...
  const gregorianDate = moment().format("MMM D");
  ...
  const hijriDay = momenthijri().iDate(); // Day of the Hijri month (1-30)
  const hijriMonthIndex = momenthijri().iMonth(); // Month index (0-11)
  const hijriYear = momenthijri().iYear(); // Hijri year

  const monthhijri = HijriCalendar.fromGregorian(date).month;
  const datehijris = HijriCalendar.fromGregorian(date).date;

  const yearhijris = HijriCalendar.fromGregorian(date).year;

  const hijriDate = `${datehijris} ${hijriMonths[monthhijri]} ${yearhijris}`;

  // const hijriDate = `${addOrdinalSuffix(hijriDay)} ${englishMonths[hijriMonthIndex]} ${hijriYear}`;
  useEffect(() => {
    if (student) {
  ...
  const remindUsersdaily = () => {
    ...
    const todayEnd = new Date();
    todayEnd.setHours(23, 59, 59, 999); // Set to 23:59:59 of today
    const filteredDaily = adaatsD?.map((aadat) => {
  ...
  const fetchAllAadats = async () => {
      ...
      const modifiedCustomfield = aadatmap?.customField?.map(
        (field, fieldIndex) => {
          if (
            ...
            const matchingCustomField = filteredtemp?.customField?.find(
              (fields) => {
                return Object.keys(fields)[0] == field.fieldTitle;
              }
            );

            const checkedValues = matchingCustomField
              ? matchingCustomField[field.fieldTitle].value
              : [];
            const updatedField = {
              ...field,
    ...
    const updatedDisplayMiqaats = displayMiqaats?.map((miqaat) => {
      const matchingMiqaat = submittedmiqaat.find(
        (submitted) => submitted.miqaatId === miqaat._id
      );

      if (matchingMiqaat && matchingMiqaat.miqaatyesno === "yes") {
        return {
          ...miqaat,
          miqaatyesno: "yes",
        };
    ...
    // if(updatedDisplayMiqaats){
    // setDisplayMiqaats(updatedDisplayMiqaats);}
  };

  const getStudentdetails = () => {
    axios
      .get(
        `${process.env.REACT_APP_DEV_BASE_URL}/api/v1/school/me?islamicDay=${datehijris}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    if (datehijris) getStudentdetails();
  }, [updateAPI, datehijris]);
  ...
  const handleImagechange = async (e, aadat) => {
    const { name, value } = e.target;
 console.log("this is a secte d event" , e.target)
    if (name.startsWith("image")) {
      // const files = e.target.files;
      // If no files are selected, clear the existing state for that input
      ...
      console.log("this is a slecte files", selectedFiles);
  ...
  const saveData = async (event) => {
    ...
    try {
      ...
      if (filteredData?.length > 0) {
        ...
        //Fd.append("aadat", JSON.stringify(newformDatanode.aadat));
        const newData = {
          ...newformDatanode, // Copy the existing data
          academicYearId: authState?.startYearId, // Add the new key
        };
  ...
  return (
    <div className="page-wrapper">
      <div className="content studentDashboard student_view">
        ...
        <div
          style={{ height: "100%" }}
          className="d-md-flex d-block mt-3 school_fix "
        >
          <div
            style={{ height: "100%" }}
            className=" d-flex justify-content-center flex-fill ps-0 border-0"
          >
            <CForm
            ...
            >
              ...
              <CRow className="justify-content-center height-innerCard ">
                <CCol xl={8} className="flex-rsponsive">
                  <div className="flex-fill">
                    <div className="card bg-dark position-relative">
                      <div className="card-body">
                        <div className="card-box-responsive marginbottom-3 mb-2 ">
                          <p
                            style={{
                              fontSize: "15px",
                              color: "white",
                        ...
                        <div className="card-box-responsive ">
                          <div className="d-flex align-items-center  row-gap-3 mb-3">
                            ...
                            <div className="d-block">
                              ...
                              <h3 className="text-truncate white-space-nowrap text-white mb-1">
                              ...
                              <div className="d-flex align-items-center flex-wrap row-gap-2 text-gray-2">
                                <span
                                  ...
                                  style={{ fontSize: "13px" }}
                                >
                                  {`Grade : ${student?.stageGradeSectionData?.grade?.grade}`}{" "}
                                  &nbsp;&nbsp;&nbsp;
                                  {`Section : ${student?.stageGradeSectionData?.section?.section}`}
                                </span>
                              </div>
                            </div>
                          </div>
                  ...
                  {student?.suratPercentage > 0 && (
                    <div className=" d-flex">
                      <div className="card flex-fill">
                        <div className="card-header d-flex align-items-center justify-content-between">
                          <h4 className="card-title">
                            Percent completed (Surat):{student.suratPercentage}%{" "}
                          </h4>
                        </div>
                        ...
                        <div className="card-body  ">
                          <div className="attendance-chart appexchartBox-respnsive">
                            <div
                              className="surat-responsive"
                              style={{ marginTop: "4px", paddingLeft: "10px" }}
                            >
                              <strong
                                className="mb-2"
                                                    ...
                                                    {(() => {
                                                      switch (type) {
                                                              ...
                                                              <div className="yes_no">
                                                                ...
                                                                adaat
                                                                  ...
                                                                  <>
                                                                    ...
                                                                    <div className="invalid-feedback selct-field yesnothing">
                                                                      Please
                                                                      select
                                                                      yes/no.
                                                                    </div>{" "}
                                                        ...
                                                        case "custom":
                                                          return (
                                                            <>
                                                              <div className="text-left customType datatype box-for-fields">
                                                                    ...
                                                                    (
                                                                      ...
                                                                      <div
                                                                        className={
                                                                          optionIndex ===
                                                                          0
                                                                            ? " form-check-inline pl-0 margin-rig"
                                                                            : " form-check-inline pl-0 margin-rig"
                                                                        }
                                                                      >
                                                              ...
                                                              <div className="d-flex align-items-center">
                                                                  {isSubmitted &&
                                                                  adaat.isCompulsory &&
                                                                  (!adaat.responsevalues?.custom && !newformDatanode?.aadat?.find(
                                                                    (item) =>
                                                                      item.aadatId ===
                                                                      adaat?._id
                                                                  )
                                                                    ...
                                                                    newformDatanode?.aadat?.find(
                                                                      ...
                                                                      ?.customType?.length ===
                                                                      0) ? (
                                                                    <div className="selct-field mtour-2">
                                                      ...
                                                      >
                                                        ...
                                                        {field.fieldType ===
                                                          ...
                                                          <div>
                                                                  ...
                                                                  option?.trim() && ( // Check if option is not blank
                                                                    ...
                                                                    >
                                                                      <input
                                                                        ...
                                                                        defaultChecked={field?.selectedFields?.includes(
                                                                          option
                                                                        )}
                                                                        value={
                                                                          option
                                                            ...
                                                            {/* {isSubmitted &&
                                                              adaat.isCompulsory &&
                                                              (!field?.selectedFields && !newformDatanode?.aadat?.find(
                                                                (item) =>
                                                                  item.aadatId ===
                                                                  adaat?._id
                                                              )?.customField ||
...
{isSubmitted &&
  ...
  (!field?.selectedFields &&
    (!newformDatanode?.aadat?.find((item) => item.aadatId === adaat?._id)
      ?.customField ||
      newformDatanode?.aadat?.find((item) => item.aadatId === adaat?._id)
        ?.customField?.length === 0)) ? (
    <div className="selct-field mtour-2">Please select a field</div>
  ) : null}
                                                      ...
                                                      case "remarkbox":
                                                        return (
                                                          <div>
                                                            ...
                                                            {/* {isSubmitted &&
                                                              ...
                                                              (!newformDatanode?.aadat?.find(
                                                                ...
                                                                newformDatanode?.aadat?.find(
                                                                  ...
                                                                  .length ===
                                                                  0) ? (
                                                                <div className="selct-field mt-2">
                                                                  Please write 
                                                                  a remark
                                                                </div>
                                                              ) : null} */}
                                                            {isSubmitted &&
  adaat.isCompulsory &&
  (!adaat.responsevalues?.remarkbox &&
    (!newformDatanode?.aadat?.find(
      (item) => item.aadatId === adaat?._id
    )?.remarkBoxes ||
      newformDatanode?.aadat?.find(
        (item) => item.aadatId === adaat?._id
      )?.remarkBoxes?.length === 0)) ? (
    <div className="selct-field mt-2">Please write a remark</div>
  ) : null}
                                                          ...
                                                          <div className="">
                                                            ...
                                                            <div key={index}>
                                                              ...
                                                              <div className="d-flex align-items-center flex-wrap row-gap-3 outline-border mb-3">
                                                                <div className="d-flex align-items-center justify-content-center avatar avatar-xxl border border-dashed me-2 flex-shrink-0 text-dark frames w-for-mobile" >
                                                                  ...
                                                                  adaat
                                                                    ...
                                                                    newformDatanode?.aadat?.find(
                                                                      ...
                                                                      (
                                                                        ...
                                                                        >
                                                                          {console.log("this is a seen------------------------------------" , newformDatanode?.aadat?.find(
                                                                (item) =>
                                                                  item.aadatId ===
                                                                  adaat?._id
                                                              )?.images)}
                                                                          <img
                                                                            crossorigin={process.env.REACT_APP_DEV_BASE_URL}
                                                                            src={
                                                                             `${process.env.REACT_APP_DEV_BASE_URL}/${src}`
                                                                            }
                                    ...
                                    /> */}
                                                              ...
                                                              adaat.isCompulsory &&
                                                              (!adaat
                                                                .responsevalues?.images && !newformDatanode?.aadat?.find(
                                                                (item) =>
                                                                  item.aadatId ===
                                                                  adaat?._id
                                                              )?.images ||
                              ...
                              {displayMiqaats?.map((miqaatItem, miqaatIndex) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div
                                      className="col-md-12 text-left mb-2 "
                                      style={{
                                        display: "flex",
                                        flexDirection: "column",
                                        gap: "10px",
